...
if __name__ == '__main__':
    ...
    if use_fake_api:
        ...
        unconverged_separations = [index for (index, item) in enumerate(converged) if item == 1]
        # iterations_without_separation = [index for (index, item) in enumerate(converged) if item == "no separation"]
        converged_separation = [index for (index, item) in enumerate(converged) if item == 0 or item ==2]
        ...
        aspen_time = []
        sim_time = [index for (index, item) in enumerate(simulation_time) if item != 0]
        non_separation_runs = [index for (index, item) in enumerate(simulation_time) if item == 0]
        ...
        if number_of_converged_separations == 0 and number_of_unconverged_separations == 0:
            ...
            print(f"Number of non separations: {len(non_separation_runs)}")
